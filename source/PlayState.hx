...
class PlayState extends MusicBeatState
{
	...
	public var sharkBites:Int = 0;

	public static var dialogueFileName:String = "dialogue";
	...
	override public function create()
	{
		...
		if (Paths.doesTextAssetExist(Paths.txt('data/songs/${PlayState.SONG.songId}/${dialogueFileName}')))
		{
			dialogue = CoolUtil.coolTextFile(Paths.txt('data/songs/${PlayState.SONG.songId}/${dialogueFileName}'));
		}
		...
		var gfCheck:String = 'kuna';

		if (SONG.songId == "interview")
		{
			gfCheck = "kuna-nospeakers";
		}

		if (!stageTesting)
		{
			gf = new Character(1038, 296, gfCheck);
			...
			if (gf.frames == null)
			{
				...
				#end
				gf = new Character(1038, 296, 'kuna');
			}

			boyfriend = new Boyfriend(770, 400, SONG.player1);
		...
		if (isStoryMode && dialogueFileName != "na")
		{
		...
		if (isStoryMode || SONG.songId == "interview")
		{
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				case 'tutorial' | 'sea-breeze' | 'whale-waltz' | 'shark-attack' | 'interview':
					schoolIntro(doof);
				default:
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			else if (dad.curCharacter == 'spooky' || dad.curCharacter == 'gf' || dad.curCharacter.startsWith('kuna'))
				dad.dance();
	...
	override public function update(elapsed:Float)
	{
		...
		if (doof != null && inCutscene && !DialogueBox.inMP4)
		{
		...
		else
		{
			...
			if (SONG.songId == "shark-attack")
			{
				// 2:00 - 1:00 - 0:00 if mania
				if ((FlxG.sound.music.time > 40000 && sharkBites == 0) || (FlxG.sound.music.time > 100000 && sharkBites == 1))
				{
					Stage.camZoom = 1.3;
					FlxG.camera.follow(new FlxObject(dad.getMidpoint().x, dad.getMidpoint().y, 1, 1), LOCKON);

					dad.playAnim("bite", true);
					FlxG.sound.play(Paths.sound('bite'), 0.5);
					...
					new FlxTimer().start(1.2, function(tmr:FlxTimer)
					{
						Stage.camZoom = 0.5;
						FlxG.camera.follow(camFollow, LOCKON, 0.04 * (30 / (cast(Lib.current.getChildAt(0), Main)).getFPS()));
						dad.playAnim("idle", true, false, 0, true);
					});
				}
			}
		...
		if (generatedMusic && currentSection != null)
		{
			...
			if (currentSection.mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				{
					case 'shark' | 'stage':
						camFollow.x = boyfriend.getMidpoint().x - 610;
	...
	function endSong():Void
	{
		if (isStoryMode)
		{
			if (SONG.songId == "tutorial")
			{
				MainMenuState.notif = "Week 1 is unlocked now!\nPress [Enter] to dismiss.";
				FlxG.save.data.finishedTut = true;
				FlxG.save.flush();
			}
			else if (SONG.songId == "shark-attack" && dialogueFileName != "ending")
			{
				dialogueFileName = "ending";
			}
			else if (SONG.songId == "shark-attack" && dialogueFileName == "ending")
			{
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist[0] != "shark-attack")
				{
	...
	override function stepHit()
	{
		...
		if (SONG.songId == "whale-waltz")
		{
			if (curStep == 67)
			{
				FlxG.camera.follow(new FlxObject(dad.getMidpoint().x, dad.getMidpoint().y - 50, 1, 1), LOCKON, 0.5);

				dad.playAnim("go", true);
		...
		else if (SONG.songId == "shark-attack")
		{
			if (curStep == 1184)
			{
				Stage.camZoom = 1.0;
				FlxG.camera.follow(new FlxObject(dad.getMidpoint().x, dad.getMidpoint().y - 180, 1, 1), LOCKON);
		...
		else if (SONG.songId == "interview")
		{
			if (curStep == 443)
			{
				Stage.camZoom = 1.2;

				dad.playAnim("rude", true);
