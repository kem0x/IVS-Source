	...
	override function update(elapsed:Float)
	{
		...
		switch (itemType)
		{
			...
			case "Vertical":
				...
				x += targetX;
			case "CVertical":
				y = FlxMath.lerp(y, (scaledY * 120) + (FlxG.height * 0.3), 0.16);
			...
			case "Diagonal":
				y = FlxMath.lerp(y, (scaledY * 120) + (FlxG.height * 0.48), 0.16);
				x = FlxMath.lerp(x, (targetY * 120) + 90, 0.16);
			case "D-Shape":
				// kill me please
				y = FlxMath.lerp(y, (scaledY * 90) + (FlxG.height * 0.45), 0.16) - 30;

				x = FlxMath.lerp(x, Math.exp(scaledY * 0.8) * 70 + (FlxG.width * 0.35), 0.16) - 10;
				if (scaledY < 0)
