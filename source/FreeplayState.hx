...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		FlxTween.tween(FlxG.camera, {zoom: 1, angle: 0}, 1.1, {ease: FlxEase.quartInOut});
		...
		domo = new FlxSprite(-80, -80).loadGraphic(Paths.image('domo'));
		//domo.screenCenter();
		add(domo);
	...
	override function update(elapsed:Float)
	{
		...
		if (domo != null
			&& FlxG.mouse.pressed
			&& FlxCollision.pixelPerfectPointCheck(Math.floor(FlxG.mouse.x), Math.floor(FlxG.mouse.y), domo))
		{
			PlayState.storyPlaylist = ["interview"];
		...
		if (controls.BACK)
		{
			FlxTween.tween(FlxG.camera, {x: -FlxG.width}, 0.5, {ease: FlxEase.quadOut});
